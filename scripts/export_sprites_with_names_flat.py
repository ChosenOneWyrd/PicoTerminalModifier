...
Examples:
    # Export all .bin files in one folder into named_sprites/vb, without manifests:
    ...
    # Preserve source subfolders / per-bin folders instead of flattening:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "When input is a folder, export every sprite directly into the "
            "output folder instead of creating one subfolder per .bin file."
        ),
    )
    parser.add_argument(
        "--no-manifest",
        action="store_true",
        help="Do not create manifest.csv files.",
    )
    parser.add_argument(
        "--recursive",
        action="store_true",
